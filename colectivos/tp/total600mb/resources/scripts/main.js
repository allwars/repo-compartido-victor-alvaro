...
tools.onStylesReady = () => { 
  ...
  const typeOfLineButton = document.querySelectorAll(
    '[data-js-vf="_buttonTypeLine"]'
  );
  const infosPortability = document.querySelectorAll(
    '[data-js-vf="_portability"]'
  );
  ...
  const navDesktop = document.querySelector('#content-nav-desktop');
  ...
  function positionOfTicket() {
    let infoBanner = banner.getBoundingClientRect();
    let heightHeader = 50;
    if(navDesktop && navDesktop.offsetHeight > 0) {
      heightHeader = navDesktop.offsetHeight;
    }
    if(infoBanner.top < 0){
      ticket.setAttribute('style', `position:fixed; top: ${heightHeader}px;`);
    } else {
...
tools.onFrameworkReady = () => {
  ...
  // Speed Selector
  const typeOfSpeedButton = document.querySelectorAll(
    '[data-js-vf="_buttonSpeed"]'
  );
  const infoRegular = document.querySelectorAll('[data-js-vf="_regularRate"]');
  const infoRecommended = document.querySelectorAll(
    '[data-js-vf="_recommendedRate"]'
  );

  const recommededCheck = document.querySelector(
    '[data-js-vf="recommendedRateCheck"]'
  );
  const regularCheck = document.querySelector(
    '[data-js-vf="regularRateCheck"]'
  );

  const queryString = window.location.search;
  console.log(queryString);
  if (queryString === "?1gbps" || queryString === "?1gbps=") {
    regularRate();
  } else {
    recommendedRate();
  }
